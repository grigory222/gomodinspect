...
| Флаг | Описание |
|------|----------|
| `--interactive` | Интерактивный режим: вводить URL репозиториев построчно. `\q` — выход, `\h` — помощь. |
| `--fast-mode` | Использовать Redis-кеш. Данные подтягиваются из кеша, если они там есть. Без флага всегда идёт запрос в сеть. |

В интерактивном режиме с `--fast-mode` фоновый воркер обновляет кеш каждые 12 часов.
